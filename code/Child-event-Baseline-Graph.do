...
*------------------------------------------------------------------------------
* Child event baseline estimation					    -- Update: 28.July.2022
*------------------------------------------------------------------------------

* Resetting
gen gender = "`gender'"
save "${hp}output/Eventstudy_baseline.dta", replace

* Getting the data by gender
foreach gender in women men  {
	use "${hp}output/`gender'_sample.dta", clear
...
* Creating Event Time Dummies
	...
	save "${hp}output/Work_eventstudy.dta", replace
...
********************************************************************************
	foreach var in earnings extensive intensive wagerate {
		...
		use "${hp}output/Work_eventstudy.dta", clear
...
********************************************************************************
		...
		append using "${hp}output/Eventstudy_baseline.dta"
...
foreach var in earnings extensive intensive wagerate {
	...
	use "${hp}output/Eventstudy_baseline.dta", clear

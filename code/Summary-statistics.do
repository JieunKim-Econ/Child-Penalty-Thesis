...
gen earning = p_wage 
replace earning = 0 if p_econstat != 1 |  p_wage == .  
// 0 if a person is not employed 

* Labor force participation: indicator variable for job status 
* 0 if individuals have zero earnings
gen lfp = (p_econstat == 1 & earning > 0) 
...
duplicates tag pid eventtime, gen(flags)
browse if flags

...
*------------------------------------------------------------------------------
* Summary statistics at one year before first childbirth -- Update: 21.July.2022
*------------------------------------------------------------------------------
...
* Create 5 income groups using the existing variable for 10 income groups "h_incomeq"
gen incgroup = .
forvalues i = 1(1)5 {
	replace incgroup = `i' if h_incomeq == 2*`i'-1 | h_incomeq == 2*`i'
}
...
sum num_pid // result: 698

by p_sex, sort: sum p_age lfp emp earning wage_rate p_hours unigrad
